...
RUN mkdir /tmp/python && \
    cd /tmp/python && \
    wget https://www.python.org/ftp/python/3.10.${PYTHON_VERSION_MINOR}/Python-3.10.${PYTHON_VERSION_MINOR}.tgz && \
    tar -xzf Python-3.10.${PYTHON_VERSION_MINOR}.tgz && \
    cd Python-3.10.${PYTHON_VERSION_MINOR} && \
...
RUN python3.10 -m venv --system-site-packages /home/perciever/.venv
...
RUN $PIP pip3 install torch torchvision --index-url https://download.pytorch.org/whl/cu118
