...
def test_cityscapes_static(split):
    from unipercept.data.sets.cityscapes import Base

    ds_cls = get_dataset("cityscapes")
    assert ds_cls is Base, ds_cls
...
def test_cityscapes_vps(split, all, pair_size):
    from unipercept.data.sets.cityscapes import DVPS

    ds_cls = get_dataset("cityscapes/vps")
    assert ds_cls is DVPS, ds_cls
    if not file_io.isdir(ds_cls.root):
        pytest.skip(f"Dataset {ds_cls.__name__} not installed @ {file_io.get_local_path(ds_cls.root)}")
