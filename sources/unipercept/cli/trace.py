...
def main(args):
    ...
    if not isinstance(submodule, torch.nn.Module):
        ...
        raise ValueError(f"Submodule model.{full_path} is not a torch.nn.Module! Got: {type(submodule)}")
...
def trace(subparser: argparse.ArgumentParser):
    ...
    subparser.add_argument("path", type=str, nargs="*", help="path to submodule to profile")
