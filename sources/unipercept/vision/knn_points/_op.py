...
from .extension import knn_points_backward, knn_points_idx

__all__ = ["KNNResult", "KNNPointsFunction", "knn_points", "knn_gather"]


class KNNResult(T.NamedTuple):
    r"""
    Attributes
    ----------
    dists:
    ...
    idx:
        LongTensor of shape (N, P1, K) giving the indices of the
    ...
    knn:
        Tensor of shape (N, P1, K, D) giving the K nearest neighbors in p2 for
...
class KNNPointsFunction(Function):
    """
...
def knn_points(
    ...
    ----------
    p1:
        Tensor of shape (N, P1, D) giving a batch of N point clouds, each
        containing up to P1 points of dimension D.
    p2:
        Tensor of shape (N, P2, D) giving a batch of N point clouds, each
        containing up to P2 points of dimension D.
    lengths1:
        LongTensor of shape (N,) of values in the range [0, P1], giving the
        length of each pointcloud in p1. Or None to indicate that every cloud has
        length P1.
    lengths2:
        LongTensor of shape (N,) of values in the range [0, P2], giving the
        length of each pointcloud in p2. Or None to indicate that every cloud has
        length P2.
    norm:
        Integer indicating the norm of the distance. Supports only 1 for L1, 2 for L2.
    K:
        Integer giving the number of nearest neighbors to return.
    version:
        Which KNN implementation to use in the backend. If version=-1,
        the correct implementation is selected based on the shapes of the inputs.
    return_nn:
        If set to True returns the K nearest neighbors in p2 for each point in p1.
    return_sorted:
        Whether to return the nearest neighbors sorted in
        ascending order of distance.
    ...
    p1_dists, p1_idx = KNNPointsFunction.apply(
    ...
    return KNNResult(dists=p1_dists, idx=p1_idx, knn=p2_nn if return_nn else None)


def knn_gather(x: Tensor, idx: Tensor, lengths: Tensor | None = None):
    """
    ...
    ----------
    x:
        Tensor of shape (N, M, U) containing U-dimensional features to
        be gathered.
    idx:
        LongTensor of shape (N, L, K) giving the indices returned by `knn_points`.
    lengths:
        LongTensor of shape (N,) of values in the range [0, M], giving the
