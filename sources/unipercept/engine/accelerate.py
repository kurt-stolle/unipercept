...
class Accelerator(accelerate.Accelerator):
    ...
    def from_engine_params(cls, params: EngineParams, root: file_io.Path) -> T.Self:
        ...
        acc = cls(
            ...
            **kwargs,
            dynamo_backend=(
                DynamoBackend(params.compiler_backend)
                if params.compiler_backend
                else DynamoBackend.NO
            ),
        )
    ...
    def _dynamo_from_params(self, params: EngineParams) -> None:
        ...
        available = sorted(map(lambda x: x.upper(), torch.compiler.list_backends()))
        backend = params.compiler_backend
        if backend is not None:
            backend = backend.upper()
            if backend not in available:
                msg = f"Invalid compiler backend '{backend}', available: {available}"
        ...
        plugin.disable = get_env(
            bool,
            "UP_ENGINE_COMPILE_DISABLE",
            default=plugin.backend == DynamoBackend.NO,
        )
