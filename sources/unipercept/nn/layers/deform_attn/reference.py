...
def deform_attn(value, value_spatial_shapes, sampling_locations, attention_weights):
    """
    ...
    """
    N, _, M, D = value.shape  # [N, S, M, D]
    assert M == sampling_locations.shape[2], (value.shape, sampling_locations.shape)

    _, Q, M, L, P, _ = sampling_locations.shape  # [N, Q, M, L, P, 2]
    ...
    attention_weights = attention_weights.transpose(1, 2).reshape(N * M, 1, Q, L * P)
    output = (
