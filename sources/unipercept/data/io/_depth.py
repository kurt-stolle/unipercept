...
from unipercept.data.points import DepthMap
from unipercept.data.types import DepthFormat

from .utils import get_kwd, read_pixels

__all__ = [
    "read_depth_map",
    "read_depth_map_from_disparity",
]

DEFAULT_DEPTH_DTYPE: Final = torch.float32


@torch.inference_mode()
@file_io.with_local_path(force=True)
def read_depth_map(path: str, dtype=DEFAULT_DEPTH_DTYPE, **meta_kwds) -> DepthMap:
    ...
    match DepthFormat(format):
        case DepthFormat.DEPTH_INT16:
            ...
            m = 1 / (m.to(torch.float64) / float(2**8))
            m = m.to(dtype=dtype, copy=False)
            m[m == torch.inf] = 0
            m[m == torch.nan] = 0
        case _:
...
@torch.inference_mode()
@file_io.with_local_path(force=True)
def read_depth_map_from_disparity(
    path: str,
    ...
    # Read disparity map
    disp = _read_image(path, mode=ImageReadMode.UNCHANGED)
