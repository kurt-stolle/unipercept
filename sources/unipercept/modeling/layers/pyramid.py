...
class DepthwiseFPN(Backbone):
    """
    FPN based on the Detectron2 implementation with depthwise convolutions.
    """
    ...
    def __init__(
        ...
        in_features,
        out_channels,
        norm: Optional[Norm] = norm.LayerNormCHW,
        top_block=None,
        fuse_type="sum",
        square_pad=0,
    ):
        ...
        for idx, in_channels in enumerate(in_channels_per_feature):
            lateral_conv = convolution.Standard2d.with_norm(
                in_channels, out_channels, kernel_size=1, bias=False, norm=norm
            )
            output_conv = convolution.Separable2d.with_norm_activation(
                out_channels,
                out_channels,
                kernel_size=3,
                stride=1,
                padding=convolution.Padding.SAME,
                bias=True,
                norm=norm,
                activation=nn.Identity,
            )

            stage_stride = strides[idx]
            assert stage_stride is not None

            stage = int(math.log2(stage_stride))
    ...
    def forward(self, x):
        ...
        input (dict[str->Tensor])
            mapping feature map name (e.g., "res5") to feature map tensor for each feature
            level in high to low resolution order.
