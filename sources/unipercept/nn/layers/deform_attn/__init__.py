...
from ._ext import deform_attn_backward, deform_attn_forward
...
class MultiScaleDeformAttn(nn.Module):
    def __init__(self, d_model=256, n_levels=4, n_heads=8, n_points=4):
        ...
        if d_model % n_heads != 0:
            msg = "d_model must be divisible by n_heads, but got {} and {}".format(
                d_model, n_heads
            )
            raise ValueError(msg)
        _d_per_head = d_model // n_heads
        if not _is_power_of_2(_d_per_head):
            msg = "d_model / n_heads must be power of 2, but got {}.".format(
                _d_per_head
            )
            raise ValueError(msg)
