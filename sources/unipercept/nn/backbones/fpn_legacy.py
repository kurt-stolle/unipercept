...
from unipercept.nn.norms import NormSpec
...
class _Combine(nn.Module):
    ...
    def forward(self, x: list[torch.Tensor]) -> torch.Tensor:
        # Resample
        nodes = []
        for i, resample in zip(self.in_offsets, self.resample, strict=False):
            n = x[i]
            n = resample(n)
            nodes.append(n)
