...
from unipercept.utils.box import boxes_to_areas
from unipercept.utils.mask import masks_to_boxes, masks_to_centers

from .constants import INDEX_DTYPE
...
class ThingAssigner(nn.Module):
    ...
    def _assign(
        ...
        *,
        ...
        # centers_index = centers.to(INDEX_DTYPE, copy=True)
        centers_index = self._scale_values(centers, scale_embed).to(INDEX_DTYPE, copy=True)
        ...
        ins_flat.categories[ass_idx] = cats.type_as(ins_flat.categories)
        ins_flat.ids[ass_idx] = iids.type_as(ins_flat.ids)
        ...
        bat_masks = torch.ones_like(select, dtype=INDEX_DTYPE).cumsum(dim=0) - 1
    ...
    def _new_instances(
        ...
        off_batch = torch.ones_like(has_lbl, dtype=INDEX_DTYPE).cumsum_(dim=0) - 1
        off_batch *= dims
        off_count = torch.ones_like(has_lbl, dtype=INDEX_DTYPE)
        off_count[~select] = 0
        ...
        ins_index = torch.ones((batch_size, dims), dtype=INDEX_DTYPE, device=device).cumsum_(dim=-1) - 1  # (B, pad_to)
        ...
        # ins = ThingInstances(
        #     indices=torch.full_like(ins_valid, -1, dtype=INDEX_DTYPE, device=device),
        #     indices_mask=ins_valid,
        #     insts=torch.full((*ins_valid.shape, *hw_embedding), self.sem_ignore, dtype=torch.float, device=device),
        #     categories=torch.full_like(ins_valid, self.sem_ignore, dtype=INDEX_DTYPE, device=device),
        #     ids=torch.full_like(ins_valid, self.sem_ignore, dtype=INDEX_DTYPE, device=device),
    ...
    def _allocate_empty(
        ...
        return ThingInstances(
            indices=torch.full(shape, -1, dtype=INDEX_DTYPE, device=device),
            ...
            categories=torch.full(shape, sem_ignore, dtype=INDEX_DTYPE, device=device),
            ids=torch.full(shape, sem_ignore, dtype=INDEX_DTYPE, device=device),
            batch_size=shape,
        )  # type: ignore
