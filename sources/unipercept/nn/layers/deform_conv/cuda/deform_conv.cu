#include "cuda/deform_conv_im2col.cuh"
#include "cuda/deform_conv_col2im.cuh"
#include <vector>
...
deform_conv_cuda_backward(
  ...
  for (int n = 0; n < batch / im2col_step_; ++n) {
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        at::ScalarType::Half, at::ScalarType::BFloat16, value.scalar_type(),
        "deform_conv_backward_cuda", ([&] {
          deform_conv_col2im_cuda(
              at::cuda::getCurrentCUDAStream(),
              value.data_ptr<scalar_t>() + n * im2col_step_ * per_value_size,
              p_offset.data_ptr<scalar_t>() +
                  n * im2col_step_ * per_offset_size,
