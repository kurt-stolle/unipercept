...
def read_config(config: ConfigParam) -> DictConfig:
    ...
    """
    from unipercept import file_io
    from unipercept.config.lazy import load_config_local, load_config_remote
...
def read_checkpoint(state: Pathable) -> dict[str, Tensor]:
    """
    ...
    match state_path.suffix:
        ...
        case ".safetensors":
            ...
            state_dict = st.load_file(state_path, device="cpu")
        case _:
            raise ValueError(
...
def load_checkpoint(state: StateParam, target: nn.Module) -> None:
    ...
    requires_grad
        Whether to set the requires_grad attribute of the parameters to the value specified. Defaults to True.
    """
    from unipercept import file_io
    from unipercept.engine import Engine
    ...
    if isinstance(state, str) or isinstance(state, os.PathLike):
        logger.info("Load checkpoint: using %s", state)
    ...
    if len(missing) > 0:
        ...
        if len(missing) > MAX_DEBUG:
            missing = missing[:MAX_DEBUG] + [
                f"... and {len(missing) - MAX_DEBUG} more",
            ]
        logger.warning("Missing weights in checkpoint: \n%s", "\n  ".join(missing))
    if len(unexpected) > 0:
        unexpected = sorted(unexpected)
        if len(unexpected) > MAX_DEBUG:
            unexpected = unexpected[:MAX_DEBUG] + [
                f"... and {len(unexpected) - MAX_DEBUG} more",
            ]
        logger.warning(
            "Unexpected weights in checkpoint: \n%s", "\n  ".join(unexpected)
        )
