...
class MetadataPathHandler(PathHandler):
    ...
    def _get_path(self, path: str, **kwargs) -> _PathlibPath:
        r"""
        Return the local path (in the temporary directory) for the given path.
        """
    ...
    def _get_traversable(self, path: str, **kwargs) -> Traversable:
        r"""
        Return the `importlib.resources` traversable object for the given path.
        """
        url = urlparse(path)
