...
model = T.cast(T.Any, model)
model.backbone = L(up.nn.backbones.fpn.FeaturePyramidBackbone)(
...
model.detector = L(multidvps.modules.Detector)(
...
model.feature_encoder = L(multidvps.modules.FeatureEncoder)(
...
model.depth_mapper = L(multidvps.modules.DepthHead)(
...
model.tracker = (L(multidvps.trackers.build_embedding_tracker)(),)
model.inference_pipeline = L(multidvps.logic.inference.InferencePipeline)(
    center_thres=0.01,
    sem_thres=0.20,
    center_top_num=200,
...
model.training_pipeline = L(multidvps.logic.training.TrainingPipeline.from_metadata)(
    name=DATASET_NAME,
    ...
    loss_segment_thing=L(up.nn.losses.WeightedThingDiceLoss)(scale=20.0),
    loss_segment_stuff=L(up.nn.losses.WeightedStuffDiceLoss)(scale=15.0),
    loss_depth_means=L(up.nn.losses.DepthLoss)(scale=1.0),
    loss_depth_values=L(up.nn.losses.DepthLoss)(scale=4.0),
    loss_reid=L(nn.TripletMarginLoss)(),
    loss_dgp=L(up.nn.losses.DGPLoss)(),
    loss_pgt=L(up.nn.losses.PGTLoss)(),
)
