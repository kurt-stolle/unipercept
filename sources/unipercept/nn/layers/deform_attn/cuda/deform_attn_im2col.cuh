...
__device__ scalar_t deform_attn_im2col_bilinear(const scalar_t* &bottom_data, 
...
template <typename scalar_t>
__device__ void deform_attn_col2im_bilinear(const scalar_t* &bottom_data, 
                                                   const int &height, const int &width, const int &nheads, const int &channels,
                                                   const scalar_t &h, const scalar_t &w, const int &m, const int &c,
...
template <typename scalar_t>
__device__ void deform_attn_col2im_bilinear_gm(const scalar_t* &bottom_data, 
                                                   const int &height, const int &width, const int &nheads, const int &channels,
                                                   const scalar_t &h, const scalar_t &w, const int &m, const int &c,
                                                   const scalar_t &top_grad,
                                                   const scalar_t &attn_weight,
...
template <typename scalar_t>
__global__ void deform_im2col_gpu_kernel(const int n,
  ...
  CUDA_KERNEL_LOOP(index, n)
  {
    ...
    for (int l_col=0; l_col < num_levels; ++l_col)
    {
      ...
      for (int p_col=0; p_col < num_point; ++p_col)
      {
        ...
        {
          col += deform_attn_im2col_bilinear(data_value_ptr, spatial_h, spatial_w, num_heads, channels, h_im, w_im, m_col, c_col) * weight;
        }
...
__global__ void deform_col2im_gpu_kernel_shm_blocksize_aware_reduce_v1(const int n,
...
template <typename scalar_t, unsigned int blockSize>
__global__ void deform_col2im_gpu_kernel_shm_blocksize_aware_reduce_v2(const int n,
                                                const scalar_t *grad_col,
                                                const scalar_t *data_value,
                                                const int64_t *data_spatial_shapes,
                                                const int64_t *data_level_start_index, 
...
template <typename scalar_t>
__global__ void deform_col2im_gpu_kernel_shm_reduce_v1(const int n,
                                                const scalar_t *grad_col,
                                                const scalar_t *data_value,
                                                const int64_t *data_spatial_shapes,
                                                const int64_t *data_level_start_index, 
...
template <typename scalar_t>
__global__ void deform_col2im_gpu_kernel_shm_reduce_v2(const int n,
                                                const scalar_t *grad_col,
                                                const scalar_t *data_value,
                                                const int64_t *data_spatial_shapes,
                                                const int64_t *data_level_start_index, 
...
template <typename scalar_t>
__global__ void deform_col2im_gpu_kernel_shm_reduce_v2_multi_blocks(const int n,
                                                const scalar_t *grad_col,
                                                const scalar_t *data_value,
                                                const int64_t *data_spatial_shapes,
                                                const int64_t *data_level_start_index, 
...
template <typename scalar_t>
__global__ void deform_col2im_gpu_kernel_gm(const int n,
                                                const scalar_t *grad_col,
                                                const scalar_t *data_value,
                                                const int64_t *data_spatial_shapes,
                                                const int64_t *data_level_start_index, 
  ...
  CUDA_KERNEL_LOOP(index, n)
  {
    ...
    for (int l_col=0; l_col < num_levels; ++l_col)
    {
      ...
      for (int p_col=0; p_col < num_point; ++p_col)
      {
        ...
        if (h_im > -1 && w_im > -1 && h_im < spatial_h && w_im < spatial_w)
        {
          deform_attn_col2im_bilinear_gm(
            data_value_ptr, spatial_h, spatial_w, num_heads, channels, h_im, w_im, m_col, c_col,
            top_grad, weight, grad_value_ptr, 
...
template <typename scalar_t>
void deform_col2im_cuda(cudaStream_t stream,
                              const scalar_t* grad_col,
                              const scalar_t* data_value,
                              const int64_t * data_spatial_shapes,
                              const int64_t * data_level_start_index,
