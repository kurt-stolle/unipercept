...
from uniutils.function import to_2tuple
...
class CondConv2d(PaddingMixin, NormActivationMixin, nn.Module):
    ...
    def __init__(
        ...
        for wd in self.weight_shape:
            weight_num_param *= wd
        self.weight = torch.nn.Parameter(torch.torch.Tensor(self.num_experts, weight_num_param))

        if bias:
            self.bias_shape = (self.out_channels,)
            self.bias = torch.nn.Parameter(torch.torch.Tensor(self.num_experts, self.out_channels))
        else:
