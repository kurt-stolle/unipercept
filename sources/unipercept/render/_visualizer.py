...
DEFAULT_LABEL_FLAGS = LabelFlags.ID
...
class Visualizer:
    def __init__(
        ...
        ----------
        img_rgb
            A numpy array of shape (H, W, C), where H and W correspond to
            the height and width of the image respectively. C is the number of
            color channels. The image is required to be in RGB format since that
            is a requirement of the Matplotlib library. The image is also expected
            to be in the range [0, 255].
        info
            Metadata of the dataset that the image originates from. Pass `None` if
            the image is not associated with any dataset, or to supress any interpretations of the data
            based on the dataset's metadata (e.g. class names).
        ...
        self._default_font_size = max(np.sqrt(self.output.height * self.output.width) // 60, 8 // scale)
    ...
    def get_mask_color(self, sem_id: int, ins_id=0):
        ...
        assert isinstance(clr, np.ndarray), f"Color of semantic class {sem_id} must be an ndarray, got {type(clr)}"
    ...
    def get_mask_label(self, sem_id, ins_id=0, labels: LabelFlags = DEFAULT_LABEL_FLAGS):
        lbl = []
        ...
        *,
        ...
        for i in range(num_instances):
            ...
            if labels is not None:
                ...
                font_size = np.clip((height_ratio - 0.02) / 0.08 + 1, 1, 2.5) * 0.5 * self._default_font_size
                self.draw_text(
                    labels[i],
                    text_pos,
                    color=lighter_color,
    ...
    def draw_text(
        ...
        *,
        ...
        self.output.ax.text(
            ...
            family="sans-serif",
            fontweight="bold",
            bbox={"facecolor": "black", "alpha": 0.9 * alpha, "boxstyle": "round,pad=0.2", "edgecolor": "none"},
    ...
    def draw_binary_mask(
        ...
        *,
        ...
        ----------
        binary_mask (ndarray):
            numpy array of shape (H, W), where H is the image height and
            W is the image width. Each value in the array is either a 0 or 1 value of uint8
            type.
        color
            color of the mask. Refer to `matplotlib.colors` for a full list of
            formats that are accepted. If None, will pick a random color.
        edge_color
            color of the polygon edges. Refer to `matplotlib.colors` for a
            full list of formats that are accepted.
        text
            if None, will be drawn on the object
        alpha
            blending efficient. Smaller values lead to more transparent masks.
        area_threshold
            a connected component smaller than this area will not be shown.
        depth_map
            depth map of the image. If None, will not draw depth label.
        depth_label_threshold
            if the mean depth of the mask is larger than this threshold, will not draw label.
        depth_label_alpha_modulation
            alpha modulation amount, e.g. how much to fade out the label at large depth.
        ...
        if not mask.has_holes:
            ...
            for segment in mask.polygons:
                ...
                self.draw_polygon(segment, color=color, edge_color=edge_color, edge_size=edge_size, alpha=alpha)
        else:
            ...
            if depth_map is not None:
                ...
                if depth_valid.any():
                    ...
                    if depth_label_alpha_modulation > 0.0:
                        label_alpha *= np.clip(1 - depth_mean * depth_label_alpha_modulation, 0.0, 1.0)

            # Draw text
            if text is not None:
                lighter_color = self._change_color_brightness(color, brightness_factor=0.2)
                self._draw_text_in_mask(binary_mask, text, lighter_color, alpha=label_alpha)
        return self.output
    ...
    def draw_polygon(self, segment, color, edge_color=None, edge_size=1.0, alpha=0.5):
        """
        Args:
        ...
        polygon = mpl.patches.Polygon(
            ...
            edgecolor=edge_color,
            linewidth=max(self._default_font_size // 12 * edge_size * self.output.scale, 1),
        )
