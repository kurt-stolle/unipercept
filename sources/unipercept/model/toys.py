...
class _SegmenterEncoder(nn.Module):
    ...
    @TX.override
    def forward(self, features: T.Dict[str, Tensor]) -> Tensor:
        return stack(
            [
...
class _SegmenterCriterion(nn.Module):
    ...
    @TX.override
    def forward(self, logits: Tensor, ground_truth: Tensor) -> T.Dict[str, Tensor]:
...
class Segmenter(ModelBase):
    ...
    @TX.override
    def select_inputs(self, data: InputData, device: T.Any) -> T.Tuple[T.Any, ...]:
